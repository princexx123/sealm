...
function render(){
  ...
  document.querySelector('#ranking-grid').innerHTML=['05','18'].map(slot=>{const item=dayRecords[slot],night=slot==='18',title=night?'Reset das 18h':'Reset das 05h',period=night?'FECHAMENTO DA TARDE':'ABERTURA DA MANHÃ';return `<article class="rank-card ${night?'night':''}"><span class="card-number">${slot}</span><div class="rank-card-head"><span class="slot-icon">${night?'☾':'☀'}</span><div><small>${period}</small><b>${title}</b></div><span class="cycle-chip"><i></i>${item?'REGISTRADO':'PENDENTE'}</span><time>${item?`Publicado ${new Date(item.published_at).toLocaleTimeString('pt-BR',{hour:'2-digit',minute:'2-digit'})}`:'Aguardando print'}</time></div><div class="screenshot">${item?`<img src="${item.image_url}" alt="Ranking de ${date}, reset das ${slot}h" data-view="${slot}">`:`<div class="empty"><div class="empty-glyph"><span>▧</span><i></i><i></i></div><b>Print ainda não publicado</b><small>O registro deste ciclo aparecerá aqui assim que for enviado.</small><div class="empty-meta"><span>◷ RESET ${slot}:00</span><span>◇ ARQUIVO DIÁRIO</span></div></div>`}</div><div class="rank-card-foot"><span>${item?'◆ SALVO PERMANENTEMENTE':'○ SEM REGISTRO PARA ESTE CICLO'}</span>${item?`<button data-view="${slot}">VER EM TELA CHEIA ↗</button>`:'<span>AGUARDANDO ATUALIZAÇÃO</span>'}</div></article>`}).join('');
}
